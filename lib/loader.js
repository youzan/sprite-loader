...
function emitSprite(name, buffer) {
  ...
  const projectOutputPath = this._compiler.options.output.path;
  const nameTpl = name || '[hash].png';
  const url = loaderUtils.interpolateName(this, nameTpl, {content: buffer});
  const theAbsoluteOutputPath = path.isAbsolute(outputPath) ? outputPath : `/${path.relative(projectOutputPath, outputPath)}`;
  const theSpriteFilePath =  path.resolve(theAbsoluteOutputPath, url);
  const theImageUrlInCSS = path.resolve(publicPath + theAbsoluteOutputPath + url);
