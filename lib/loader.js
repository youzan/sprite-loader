...
function emitSprite(name, buffer) {
    ...
    return this._compiler.options.output.publicPath + name;
}
    ...
    co(function* () {
        ...
        while (groups.length) {
            ...
            if (group.ratio == PixelRatio.X2) {
                ...
                let file2x = emitSprite.call(loader, hash + '@2x.png', meta2x.image);
                rule2x = {
                    type: 'rule',
                    declarations: [{
                        type: 'declaration',
            ...
            let file = emitSprite.call(loader, hash + '.png', meta.image);
            let groupSelectors = [];
