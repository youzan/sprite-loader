...
function create1x(meta) {
    ...
    return new Promise(function (resolve, reject) {
        lwip.open(meta.image, 'png', function (err, image) {
            if (err) {
                reject(err);
            } else {
                image.resize(properties.width, properties.height, function (err) {
                    if (err) {
                        reject(err);
                    } else {
...
function spriteIgnore(rules) {
    return _.find(rules, r => r.type == 'comment' && r.comment.indexOf('sprite-loader-ignore') > -1);
}
...
module.exports.loader = function (content) {
    ...
    if (spriteIgnore(ast.stylesheet.rules)) {
        return content;
    }
    ...
    let tempDir = path.join(this._compiler.context, TEMP_DIR);
    co(function* () {
        let extraRule = [];
